...
def show_pipeline():
    ...
    if not st.session_state.pipeline_logged_in:
        ...
        st.subheader("🔐 Pipeline Login")
        pw = st.text_input("Pipeline Passwort", type="password")

        if st.button("Login Pipeline"):
    ...
    st.success(f"Region: **{region.upper()}**")
    ...
    if st.session_state.show_lead_form:
        ...
        with st.form("lead_form"):
            ...
            ergebnis = st.selectbox(
                "Ergebnis",
                ["Lost", "Hot", "Follow up booked", "Book follow up"]
            )
            next_action = st.date_input("Next action", value=date.today())
            notes = st.text_area("Notes")

            submit = st.form_submit_button("💾 Lead speichern")
    ...
    if df.empty:
        ...
        st.dataframe(
            df[
                ["Last Contact", "Generated by", "AM/NB", "Name",
                 "Adresse", "Ergebnis", "Next Action", "Notes"]
            ],
