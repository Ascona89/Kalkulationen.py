...
def show_radien():
    ...
    # Radien
        ...
        try:
            response = requests.get("https://photon.komoot.io/api/",
...
def show_pipeline():
    ...
    if st.session_state.show_lead_form:
        with st.form("lead_form"):
            ...
            assign_ma = st.selectbox("Lead zuweisen an MA", [f"MA {i}" for i in range(1, ma_count + 1)], index=0)
    ...
    if search_address.strip():
        ...
        if loc:
            ...
            # Neue Karte für gefilterte Leads
            m = folium.Map(location=[search_lat, search_lon], zoom_start=10)
            folium.Circle([search_lat, search_lon], radius=search_radius*1000, color='blue', fill=True, fill_opacity=0.2).add_to(m)
            for r in filtered:
                folium.Marker(
                    [r['lat'], r['lon']],
                    tooltip=f"{r['name']} ({r['ergebnis']})",
