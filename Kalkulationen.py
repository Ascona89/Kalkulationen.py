...
def show_contractnumbers():
    ...
    st.write(f"Web Shop: {(shop['MRR_Woche'] if shop is not None else 0):.2f} €")
    st.write(f"App Shop: {(app['MRR_Woche'] if app is not None else 0):.2f} €")
    st.write(f"Shop Anmeldegebühr: {(shop['OTF'] if shop is not None else 0):.0f} €")

    st.markdown("### POS")
    st.write(f"POS Abonnementgebühr: {(pos['MRR_Woche'] if pos is not None else 0):.2f} €")
    st.write(f"POS Anmeldegebühr: {(pos['OTF'] if pos is not None else 0):.0f} €")
    st.write(f"TSE: {(tse['MRR_Woche'] if tse is not None else 0):.2f} €")

    st.markdown("### YOYOPAY")
    pay_daily = (pay["MRR_Monat"] / 30) if pay is not None else 0
    st.write(f"Tägliche Abonnement-Festgebühr: {pay_daily:.2f} €")
    st.write(f"Feste Anmeldegebühr: {(pay['OTF'] if pay is not None else 0):.0f} €")

    st.markdown("### KIOSK")
    kiosk_daily = (kiosk["MRR_Monat"] / 30) if kiosk is not None else 0
    st.write(f"YOYOKiosk-Einrichtungsgebühr: {(kiosk['OTF'] if kiosk is not None else 0):.0f} €")
    st.write(f"YOYOKiosk wöchentliche Festgebühr: {kiosk_daily:.2f} €")

    st.markdown("### Hardware")

    def hw_line(label, key):
        row = hw.get(key)
        if row is None or row["Menge"] == 0:
            return
        gesamt = row["OTF"]
        menge = int(row["Menge"])
        einzel = gesamt / menge if menge else 0
        st.write(f"{label}: {gesamt:.0f} € ({menge}x {einzel:.0f} €)")

    hw_line("SunMi D3 Pro", "POS inkl 1 Printer")
    hw_line("Kundendisplay", "Additional Display")
    hw_line("Cash Drawer", "Cash Drawer")
